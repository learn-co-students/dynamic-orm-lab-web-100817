  ...
  context 'has instance methods to insert data into db' do
    ...
    describe '#save' do
      it 'saves the student to the db' do
        new_student.save
        # having the self.col_values_for_insert like "'Sam','11'"
        # is adding the string as a whole( name = 'Sam','11') instead of
        # adding 'Sam' into name and '11' into grade.
        #One way to solve this is to remove the double quotes and just
        #adding it to one individual element instead of the group, by solving
        #the interpolation and adding the string as literal to the sql string
        #other is to add quotes only to the name and keep grade without it, since
